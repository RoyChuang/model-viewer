...
| [src/app/api/model/\[id\]/route.ts](src/app/api/model/%5Bid%5D/route.ts) | 驗證 token 後才解密回傳 |
| [src/workers/decrypt.worker.ts](src/workers/decrypt.worker.ts) | 先取 token，再帶 token 取模型 |
